...
struct Item: Codable, Inflatable {
	...
	init() {
		self.arg = nil
		self.uid = nil
		self.icon = ["path":"images/icons/archive.chat.png"]
		self.valid = true
		self.title = ""
		self.subtitle = ""
		...
		let response: Response = {
			guard !items.isEmpty else {
				return Response(items: [.with({
					$0.title = "No results."
					$0.valid = false
					$0.icon = ["path":"images/icons/info.png"]
				})])
			}
			return self
		}()
...
struct Modifier: Codable {
	...
	init(
		arg: String = "",
		subtitle: String? = nil,
		icon: [String:String] = ["path":"images/icons/info.png"],
		...
		if let query: String {
			...
			guard let firstMatch: String = messages.dropFirst()
						...
						.content
				{
					...
					if let service: String = messages.first(where: { $0.service != nil })?.service {
						item.icon = ["path":"images/icons/service.\(service).png"]
					}
